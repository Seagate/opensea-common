...
eReturnValues replace_File_Name_In_Path(char fullPath[OPENSEA_PATH_MAX], char* newFileName)
{
    ...
    // now that we have a valid pointer, set all the remaining characters to null,
    // then set the new file name in place.
    memset(ptr, 0, ptrLen);
                                             ...
                                             fileUniqueIDInfo* uniqueIdInfo /*optional*/)
{
    ...
    if (fileInfo && filename && mode) /* Not checking extList and outInfo because it is optional */
    {
        ...
        // Since we may not always be in a system with C11 or support for wx/w+x,
    ...
    !(defined(USING_MUSL_LIBC) && USING_MUSL_LIBC > 0) // Glibc supports x. VS2013+ support x
            // For systems that do not support 'x' need to adjust the mode manually so
            // that we do not pass an invalid 'x' flag when it is not supported.
            //       While this can always be done regardless of standard/library, it
            //       is better to pass flags onwards when they are supported for
            //       additional verification by the library/system - TJE
            duplicatedModeForInternalUse = true;
            internalmode                 = strdup(mode);
            char* thex                   = strchr(internalmode, 'x');
            if (thex)
            {
                // remove it since it is not supported outside C11 and a few \libraries
        ...
        if (creatingFile)
        {
            // setup path to the directory to make sure it exists since the file does
            // not yet. NOTE: If we only have the current file name, no relative path,
            // etc, need to call a function to get the current working directory to
            //       use that as the path.
            if (strchr(filename, '/') // always check for forwards slash since Windows
                                      // can accept this
#if defined(_WIN32)
                || strchr(filename, '\\') // only check for backslash on Windows since
                                          // this is a common path seperator on
                                          // Windows and is the "prefered" format
#endif                                    //_WIN32
            )
            {
                // there is a path or relative path that can be used to setup for
                ...
                {
                    // backslash was detected last, so change to this pointer instead for
                    // strndup
                    lastsep = lastwinsep;
                }
#endif //_WIN32
            ...
            else
            {
                ...
                // get the current working directory to use for the canonical file and
                // path. Will use getcwd to get this information. NOTE: A common
                // extension to getcwd is passing M_NULLPTR will have it allocate and
                // return a buffer.
                //       This seems to exist on Linux, FreeBSD, Windows (_getcwd),
                //       openbsd, netbsd, and solaris...possibly more. While this is
                //       convenient, we are not using this to make sure behavior is as
                //       expected according to POSIX
        ...
        {
            // not creating a new file, so we will have what we need to get a full
            // canonical path
        ...
        if (creatingFile)
        {
            ...
            {
                // there is a path or relative path that we do not want to append to the
                // canonical path
                char* lastsep = strrchr(filename, '/');
#if defined(_WIN32)
                // In Windows, we need to determine if the final seperator is a / or \.
        ...
        {
            // Validate the file extension if one was provided to check for since the
            // full path will also resolve the extension.
            //         This check does not guarantee a file is correct since anyone
            //         can rename an extension, but it should help make sure the file
            ...
            // Do a first pass on file attribute comparison with the expected file
            // attributes. If no match, return an error This may be done again later
            // after opening the file since some things will not be available without
            // opening the file handle.
            if (expectedFileInfo)
            {
                // check device ID and inode. These are unique on POSIX as a file ID.
                // Windows is not far enough along to get this info yet.
                ...
                // Convert the strings back to structures and use EqualSid and EqualAcl
                // to do the comparisons. There may be other formatting in the string
                // that does not affect the permissions which is why the conversion is
                // necessary.
        ...
        // Canonical path will already have the proper system path separator in it.
        // No need to search / in Windows
        fileInfo->filename =
            strrchr(fileInfo->fullpath, SYSTEM_PATH_SEPARATOR) + 1; // plus 1 to get past final seperator

        // Need to verify only the path. Passing the file in with it will cause it
        // to fail since it is not a directory
        ...
        // In Windows, we need to determine if the final seperator is a / or \.
        // So also check for \ and figure out which was the last one.
        // A user can pass a path with both and it can be accepted by Windows, which
        // is why we validate both of these.
...
#endif //_WIN32
        pathOnly = strndup(fileInfo->fullpath, C_CAST(uintptr_t, lastsep) -
                                                   C_CAST(uintptr_t, fileInfo->fullpath)); // path only. No file name
        if (pathOnly)
        {
...
#    endif //!_WIN32
...
#endif // DISABLE_SECURE_FILE_PATH_CHECK || _WIN32
        {
            ...
            {
                // file is opened, check unique id if it was passed in to compare
                // against
                ...
                if (uniqueIdInfo)
                {
                    ...
                    {
                        // This is not the same file as was meant to be opened. Close it and
                        // return an error.
                ...
                if (expectedFileInfo)
                {
                    // devid and inode already checked, but if user didn't provide unique
                    // id, it will not hurt to check again
                    ...
                    // EqualAcl to do the comparisons. There may be other formatting in
                    // the string that does not affect the permissions which is why the
...
M_NODISCARD eSecureFileError secure_Seek_File(secureFileInfo* fileInfo, offset_t offset, int initialPosition)
{
    if (fileInfo)
    {
        ...
        {
            int seekres = 0;
            // Windows has _fseeki64, which may be better to use instead for larger
            // files or to be compatible with larger files. Linux/posix have fseeko
            // and ftello which use off_t which can be wider as well. (POSIX 2001)
       ...
       // validate permissions of the directories.
        ...
        else if (os_File_Exists(fullpath))
        {
            ...
            case SEC_DELETE_NAME_UNLINK_IF_OPEN:
                // File cannot be opened, so we must assume something else has it open,
                // so unlink instead-TJE
#if defined(_WIN32)
