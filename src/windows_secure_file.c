...
static bool win_Get_File_Security_Info_By_Name(const char* const filename, fileAttributes* attrs)
{
    ...
    if (filename && attrs)
    {
        ...
        // Do not need SACL since it may not be available and is about triggering
        // audits instead of a dacl which defines who has access to a file.
                                                  ...
                                                  M_NULLPTR, &secDescriptor))
        {
            ...
            if (TRUE == ConvertSecurityDescriptorToStringSecurityDescriptorA(secDescriptor, SDDL_REVISION, secInfo,
                                                                             &temp, &tempLen))
            {
                /* do not use strdup or strndup here. There are some extra nulls at the
                 * end of what Windows allocates that we are preserving in memcpy */
...
M_NODISCARD fileUniqueIDInfo* os_Get_File_Unique_Identifying_Information(FILE* file)
{
    if (file)
    {
        ...
        {
            return M_NULLPTR; // This is stdout, strerr, stdin and not a file that we
                              // can get other info on. This is not the correct thing
        ...
        if (is_Windows_Vista_Or_Higher()) // This ex function is only available on
                                          // Vista and later OSs according to MSFT
                                          // docs
        {
            // try to get ex file info, then if it fails (shouldn't happen) then fall
            // back to old method to get this
...
static char* win_dirname(char* path)
{
    if (path)
    {
        ...
        if (lastseperator)
        {
            ...
            {
                /* Special case. The root path C:\ needs to leave the trailing backslash
                 * behind */
                *lastseperator = '\0';
            }
            return path;
        ...
        {
            // previously returned ".", but this case does not appear to be hit so
            // changing to NULLPTR so that we do not need a const_cast-TJE
            return M_NULLPTR;
        }
    }
    else
...
static bool is_Secure_Well_Known_SID(PSID sid, bool allowUsersAndAuthenticatedUsers)
{
    ...
    if (sid && IsValidSid(sid))
    {
            ...
            IsWellKnownSid(sid, WinNtAuthoritySid) || IsWellKnownSid(sid, WinBuiltinAdministratorsSid))
        {
            ...
            //  POSIX validates user or root. Do any of these others below in the if
            //  above as accepted???
            // WinLocalSid
        ...
        {
            // This is a special case for C: and C:\Users where these accounts/groups
            // also has permission and must be trusted.
            secure = true;
        }
    }
    return secure;
...
static bool get_System_Volume(char* winSysVol, size_t winSysVolLen)
{
    ...
    else
    {
        ...
        {
            // modify this info to be just the volume as we need to know this, not
            // C:\Windows. It MAY already be set to C:\, but it may not.
            ...
            {
                // need to add a null after this backslash so it's set to only the
                // volume in the path. This root directory is necessary to know to
                // properly validate other permissions.
                *(slashPointer + 1) = '\0';
            }
            validsystemvol = true;
            memset(winSysVol, 0, winSysVolLen);
    ...
    do
    {
        ...
        if (!EqualSid(mySid, userSid) && !is_Secure_Well_Known_SID(userSid, false))
        {
            if (get_System_Volume(windowsSystemVolume, MAX_PATH))
            {
                ...
                {
                    const char* WindowsTrustedInstallSID =
...
#if defined(_DEBUG)
                    printf("This path is on the system drive. Set flags for only "
                           "specific folders to validate permissions correctly\n");
#endif //_DEBUG
       ...
       //      listed here.
                    if (strcmp(dirptr, windowsSystemVolume) == 0)
                    {
                        ...
                        {
                            // This case the Windows trusted installer is found and can be
                            // trusted. This should only happen on the system root directory
                            // (C:) in Windows
#if defined(_DEBUG)
                            printf("Detected Windows trusted installer as owner of this "
                                   "directory and allowed to trust it.\n");
#endif                                                              //_DEBUG
        ...
        else
        {
            ...
            if (get_System_Volume(windowsSystemVolume, MAX_PATH))
            {
                ...
                if (strstr(dirptr, windowsSystemVolume) == dirptr)
                {
                    // need to construct the paths to allow based on the windows volume
                    // path read earlier Using MAX_PATH for each of these for simplicity.
                    // Can reduce memory by shortening, but this is fine for now on this
                    // limited list.
                    ...
                    common_String_Concat(usersdir, MAX_PATH,
                                         "Users"); // no trailing slash as the paths passed in are not
                                                   // completed with a slash
...
static bool internal_OS_Is_Directory_Secure(const char* fullpath, unsigned int num_symlinks)
{
    ...
    if (!secure)
    {
        ...
        // so use it + 1 as the starting point to go through and cleanup the stored
        // directories to free up memory
        ...
        if (attrs->fileFlags & FILE_ATTRIBUTE_REPARSE_POINT)
        {
            ...
            if (link != INVALID_HANDLE_VALUE)
            {
                PREPARSE_DATA_BUFFER reparseData =
                    C_CAST(PREPARSE_DATA_BUFFER, safe_malloc(sizeof(REPARSE_DATA_BUFFER) + MAX_PATH));
                if (reparseData)
                {
                    DWORD bytesRead = 0;
                                        ...
                                        &bytesRead, M_NULLPTR))
                    {
                        ...
                        if (bufferSize > 0)
                        {
                            ...
                            {
#if defined(__STDC_SECURE_LIB__)
                                wcstombs_s(M_NULLPTR, reparsePath, bufferSize,
                                           C_CAST(wchar_t*, reparseData->SymbolicLinkReparseBuffer.PathBuffer),
                                           bufferSize);
#else
                                wcstombs(reparsePath,
                                         C_CAST(wchar_t*, reparseData->SymbolicLinkReparseBuffer.PathBuffer),
                                         bufferSize);
#endif
                                num_symlinks++;
...
eReturnValues get_Full_Path(const char* pathAndFile, char fullPath[OPENSEA_PATH_MAX])
{
    ...
    {
        // This case is when the function returns a buffer size that is required to
        // hold the full path name. DWORD lastError = GetLastError();
        return FAILURE;
    }
#if defined(UNICODE)
    // NOTE: Microsoft accepts either ls (standard) or ws (microsoft specific).
    //       If this causes any warnings in MSVC, add an ifdef to use ws for that
    //       compiler.-TJE Microsoft also has a S extension for strings to specify
    //       single char with wprintf and wide char for printf that may also be
    //       used as needed-TJE
