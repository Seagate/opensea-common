...
static uint32_t byteswap_uint32(uint32_t dword)
{
    ...
    // Windows defines uint32 as unsigned int, not unsigned long as this function
    // expects. So we have a more elaborate check in order to make sure these have
    // the same range before casting and returning that result It is unlikely this
    // will change in Windows, but it does not hurt to be safe and verify
    // everything before casting types that can de defined as different widths-TJE
...
genericint_t generic_Get_Bit_Range(genericint_t input, size_t outputsize, uint8_t msb, uint8_t lsb)
{
    ...
    else
    {
        ...
        case sizeof(uint8_t):
            ...
            {
                // output size is not used in this case since a uint8_t can only ever
                // output a uint8_t
                out.sizeoftype = sizeof(uint8_t);
                out.u8 =
                    ((input.u8) >> (lsb)) &
                    ~(~UINT32_C(0) << ((msb) - (lsb) + UINT8_C(1))); // starting with UINT32_C to resolve warning about
                                                                     // signed int since UINT8_C() does not apply U
        ...
        case sizeof(uint16_t):
            ...
            else // default case where outputsize is not less than uint16
            {
                ...
                out.u16 =
                    ((input.u16) >> (lsb)) &
                    ~(~UINT32_C(0) << ((msb) - (lsb) + UINT8_C(1))); // starting with UINT32_C to resolve warning about
                                                                     // signed int since UINT16_C() does not apply U
                out.issigned = input.issigned;
            }
            break;
            ...
            else if (outputsize == sizeof(uint8_t))
            {
                ...
                else
                {
                    ...
                    out.u8         = ((input.u64) >> (lsb)) &
                             ~(~UINT64_C(0)
                               << ((msb) - (lsb) + UINT8_C(1))); // starting with UINT32_C to resolve warning about
                                                                 // signed int since UINT16_C() does not apply U
                    out.issigned = input.issigned;
                }
            }
