...
// NOTE: Do not use the DECLAR_ZERO_INIT_ARRAY here as it will not compile
...
uint64_t get_Milliseconds_Since_Unix_Epoch(void)
{
    ...
    // not work. This apparently causes an error in mingww64 environment in msys2
    // since this function is missing, but C11 is defined.
#if defined(USING_C11) && defined(TIME_UTC)
    struct timespec now;
    memset(&now, 0, sizeof(struct timespec));
    if (0 != timespec_get(&now, TIME_UTC))
    {
        // NOTE: Technically this is a time since the system's epoch as C standard
        // does not set an epoch.
        //       I'm not sure there is a system we have encountered where it is not
        //       the Unix epoch, so this is fine for now - TJE
        msSinceJan1970 =
...
#endif // C11
    {
        ...
        // NOTE: Using clock_gettime since it's more accurate and not affected by
        // time-skew
        ...
        // Use the function in the link below, but MSFT also documents another way
        // to do this, which is what we've implemented -TJE
        ...
        memcpy(&nowAsInt, &ftnow,
               sizeof(ULARGE_INTEGER)); // these are both 8 bytes in size, so it SHOULD
                                        // be fine and MSFT says to do this-TJE
        SystemTimeToFileTime(&epoch, &epochfile);
        memcpy(&epochAsInt, &epochfile,
               sizeof(ULARGE_INTEGER));               // these are both 8 bytes in size, so it SHOULD
                                                      // be fine and MSFT says to do this-TJE
        if (nowAsInt.QuadPart >= epochAsInt.QuadPart) // this should always be true, but seems like microsoft
                                                      // is likely checking this, so checking this as well-TJE
        {
            msSinceJan1970 = C_CAST(uint64_t, (nowAsInt.QuadPart - epochAsInt.QuadPart)) /
                             UINT64_C(10000); // subtracting the 2 large integers gives 100 nanosecond
                                              // intervals since jan 1, 1970, so divide by 10000 to get
                                              // milliseconds (divide by 10000000 to get seconds)
        }
        else
#endif // OS unique methods to get this info
        {
            // time_t is not guaranteed to always be a specific number of units. This
            // can be system specific/library specific. so to convert using old
            // standards, take a current time_t, convert it to struct tm, then
            // calculate it. If time_t is 32bits, then this will not work past 2038.
            // To detect this, we just need to check if the current year is less than
            // 1970. If it is, it could be a roll over problem or some other clock
            // issue. in that case, just return 0. NOTE: This code seems to work until
            // approximately the year 275705. The following year I've noticed can be
            // off by a day. There is probably a missing
            //       adjustment factor somewhere else, but this is close enough-TJE
            // NOTE: To take into account leap seconds you need a table, or you can
            // average 1 every 1.5 years:
            ...
            {
                // need to account for leap years before calculating below. This will
                // get how many leap years there are between the current year and 1970
                // to add to number of days for adjustment
                uint64_t leapyears = (currentyear - UINT64_C(1970)) / UINT64_C(4);
                // now subtract the number of skipped leap years (every 100 years unless
                // divible by 400). This needs to be number since 1970
                uint64_t skippedleapyears = UINT64_C(0);
                // years 2000 and 2400 are leap years, but 1700, 1800, 1900, 2100, 2200,
                // and 2300 are not. To get skipped leap years, need to calculate based
                // on number of centuries that have passed since 1970, then check how
                // many have skipped leap year. No need to worry about this until at
                // least 2100, however if you skip the year 2000, this causes a problem
                // in 24606 where it will be off by 1 day.-TJE
                if (currentyear >= UINT64_C(2000)) // start at 2000 since it is the
                                                   // first century after 1970.
                {
                    // at least year 2100, so need to handle skipped leap years and how
                    // many of these have occurred
                    uint64_t centuriesSince1970 = ((currentyear - UINT64_C(1970)) / UINT64_C(100)) - UINT64_C(1);
                    // use a loop to check each century, starting in 2100 to see if it was
                    // a skipped leap year or not.
                ...
                // Need to detect if the leap year is in the current year and if it has
                // occured already or not.
                if (currentyear % UINT64_C(4) == UINT64_C(0))
                {
                    bool adjustforcurrentleapyear = true;
                    // leap year is in the current year, unless it is a year to skip a
                    // leap year (every 100 years, unless divisible by 400)
                    if (currentyear % UINT64_C(100) == UINT64_C(0) && currentyear % UINT64_C(400) != UINT64_C(0))
                    {
                        // currently in a century year without a leap year. No need for the
                        // adjustment below
                    ...
                    if (adjustforcurrentleapyear)
                    {
                        ...
                        if (nowstruct.tm_mon < 2)
                        {
                            ...
                            {
                                // subtract 1 from leapyears since it has not yet occurred for
                                // the current year
            ...
            {
                // some weird time skew issue or 32bit time_t 2038 issue, so just return
                // zero for an invalid value.-TJE NOTE: with a 32bit time_t you could
                // probably implement a clever workaround to adjust it to a correct
                // year, but that is not worth
                //       implmenting in this code. I don't even know which modern OS is
                //       using a 32bit time_t in 2024. -TJE If a workaround to adjust
                //       the year was implemented, it would likely only work until some
                //       future time where the same 32bit problem happens again.-TJE
...
// posix definitions or C standard versions, or possible compiler specific
...
struct tm* get_UTCtime(const time_t* timer, struct tm* buf)
{
    ...
    if (timer && buf)
    {
        ...
        // If MSFT CRT available, use microsoft gmtime_s which is incompatible with
        // the standard
...
#else
        // There may be better options beyond what is done below, but it may be a
        // per-system implementation or something like that
        //       So there is room for improvement in this fallback function
        // Last thing we can do is use the unsafe version and copy it immediately to
        // our own buffer if nothing else is possible
...
#elif defined(__STDC_SECURE_LIB__)
        // If MSFT CRT available, use microsoft localtime_s which is incompatible
        // with the standard
...
char* get_Time_String_From_TM_Structure(const struct tm* timeptr, char* buffer, size_t bufferSize)
{
    if (timeptr && buffer && bufferSize >= TIME_STRING_LENGTH)
    {
        ...
        // matching output for this function asctime (which this replaces uses the
        // format: Www Mmm dd hh:mm:ss yyyy\n) NOTE: %e is C99. C89's closest is %d
        // which has a leading zero. To be technically correct in replacing this,
        // need to set locale to english instead of localized locale first. After
        // that is done, it can be reverted back.
...
#    endif
        {
            // This means the array was too small...which shouldn't happen...but in
            // case it does, zero out the memory
            memset(buffer, 0, bufferSize);
        }
        if (restoreLocale)
        {
