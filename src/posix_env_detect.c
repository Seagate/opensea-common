...
static int lin_file_filter(const struct dirent* entry, const char* stringMatch)
{
    ...
    if (filename)
    {
        ...
        {
            if (S_ISREG(s.st_mode)) // must be a file. TODO: are links ok? I don't think
                                    // we need them, but may need to revisit this.
            {
...
{
    return lin_file_filter(entry, "version"); // most, but not all do a _version, but some do a -, so
                    ...
                    if (releaseSize > 0)
                    {
                        ...
                        if (etcFileMem)
                        {
                            if (fread(etcFileMem, sizeof(char), C_CAST(size_t, releaseSize), release) !=
                                ...
                                ferror(release))
                            {
                                // some error occurred reading the file, so free this memory to
                                // return a checkable null pointer.
...
static bool get_Linux_Info_From_Distribution_Specific_Files(char* operatingSystemName)
{
    ...
    if (operatingSystemName)
    {
        ...
        // implemented: http://linuxmafia.com/faq/Admin/release-files.html This may
        // not cover all of the possible Linux's, but it should get most of them.
        // We'll check for the most common ways of specifying the file to keep this
        // simpler. NOTE: This may also work on some other unix systems, so we may
        // want to try it on those as well.
        ...
        if (releaseFileCount > 0)              // ideally this will only ever be 1
        {
            for (int releaseIter = 0; releaseIter < releaseFileCount; ++releaseIter)
            {
                if (0 == strcmp(osrelease[releaseIter]->d_name, "lsb-release"))
                {
                    ...
                    // Don't read this file for version info yet. it's common across
                    // systems but the other release and version files are more
                    // interesting to us
                }
                else
                {
                ...
                if (gotLinuxInfo)
                {
                    // we got a name, so break out of the loop (in case there are multiple
                    // files, we only need to read one)
                    break;
                }
            }
        }
        ...
        {
            // this case means that we found the lbs-release file, but haven't read it
            // yet because we kept searching for other files to use for version
            // information first. So now we need to read it for the version
            // information. We use this last because it may not contain something as
            // friendly and useful as we would like that the other version files
            // provide.
        ...
        // clean up the memory allocated for the list of version files and release
        // files.
        ...
        if (gotLinuxInfo)
        {
            // remove any control characters from the string. We don't need them for
            // what we're doing
            ...
            // set the null terminator at the end to make sure it's there and not
                                       ...
                                       uint16_t    versionCount)
{
    if (verStr && validVerSeperators && versions && versionCount > 0)
    {
        ...
        char* strscan = M_CONST_CAST(char*, verStr); // removing const in order to allow changing strscan
        ...
        {
            errno = 0; // clear this before we begin to make sure it isn't holding
                       // some previous error-TJE
            unsigned long value = strtoul(strscan, &endptr, 10);
            if (strscan == endptr)
            {
                // When this happens, the value read is zero and the conversion failed
                // returning a zero. Most likely a prefix was missing to move past
                // above.
                break;
            }
            ...
            {
                // This expects all version numbers to be uint16_t, so if this is above
                // that limit, return a failure
                errno   = ERANGE;
                success = false;
                break;
            }
            else
            {
                ...
                versionoffset += 1;
                strscan = endptr;
                // now check if endptr is at the end of the string, or is a valid
                // version seperator to move past
                if (endptr)
                {
                    size_t iter     = 0;
                    bool   validsep = false;
                    ...
                    {
                        // move past the seperator. There should only be 1 at a time with
...
eReturnValues get_Operating_System_Version_And_Name(ptrOSVersionNumber versionNumber, char* operatingSystemName)
{
    ...
    // check what is filled in by the uname call in the utsname struct (BUT DON"T
    // CHECK THE DOMAIN NAME since that is GNU specific)
    ...
    if (0 == uname(&unixUname))
    {
        ...
        if (strcmp("LINUX", unixUname.sysname) == 0) // some OSs like Android or ESX may fall into here, so this section
                                                     // may need expansion if that happens
        {
...
#define LINUX_OS_INFO_COUNT_MAX_METHODS_TO_ATTEMPT (3)
            ...
            // kernel.major.minor-securityAndBugFixes-SomeString older linux kernels
            // don't have the -securityAndBugFixes on the end
        ...
        else if (strcmp("AIX", unixUname.sysname) == 0)
        {
            ...
            versionNumber->versionType.aixVersion.majorVersion =
                C_CAST(uint16_t, strtoul(unixUname.version, M_NULLPTR, 10));
            versionNumber->versionType.aixVersion.minorVersion =
                C_CAST(uint16_t, strtoul(unixUname.release, M_NULLPTR, 10));
            if (operatingSystemName)
            {
...
#        endif
            if (dataSize == -1)
            {
                ...
                dataSize = 1024; // start with this, will increment it below if it fails
                                 // to read
            }
            ...
            if (rawBuffer)
            {
                ...
                {
                    // this means there was not enough memory allocated in order to read
                    // this. realloc with more memory and try again
                    char* temp = M_NULLPTR;
                    dataSize *= 2;
                ...
                if (error == 0 && userInfo && rawBuffer)
                {
                    ...
                    if (userNameLength > 1 &&
                        (userNameLength - 1) <= get_Sys_Username_Max_Length()) // make sure userNameLength is
                                                                               // valid and not too large
                    {
...
#    else  // defined (POSIX_2001) && defined(_POSIX_THREAD_SAFE_FUNCTIONS)
            ...
            // This structure can be in a static location, so writing zeroes to it
            // might be a good way to make sure this is cleared out after we are done.
            // the docs online say subsequent calls to getpwuid may change it, making
            // this not thread-safe, so I would assume this is ok to do.
            explicit_zeroes(userInfo, sizeof(struct passwd));
