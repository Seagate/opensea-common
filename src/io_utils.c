...
#    if defined(va_copy)
    va_copy(countargs,
            args); // C99, but available in VS2013 which is the oldest VS compiler
                   // we expect to possibly work with this code.
#    else
...
#    endif
    ...
    if (charCount == -1)
    {
        ...
        {
            // out of space or some error occurred, so need to null terminate and
            // calculate how long the buffer should have been for this format
            if (buffer && bufsz > 0)
            {
                // null terminate at bufsz since there was no more room, so we are at
                // the end of the buffer already.
                buffer[bufsz - 1] = '\0';
            }
...
eReturnValues get_Simple_Text_Output_Protocol_Ptr(void** pOutput)
{
    ...
    if (!gBS) // make sure global boot services pointer is valid before accessing
              // it.
    {
...
void set_Console_Colors(bool foregroundBackground, eConsoleColors consoleColor)
{
    ...
    if (!defaultsSet)
    {
        // First time we are setting colors backup the default settings so they can
        // be restored properly later.
...
static eKnownTERM get_Terminal_Type(void)
{
    ...
    if (get_Environment_Variable("TERM", &termEnv) == ENV_VAR_SUCCESS && termEnv != M_NULLPTR)
    {
        ...
        {
            // alpine linux does not set COLORTERM or anything else, so this would be
            // complete there, but let the other checks run too linux kernel 3.16 and
            // earlier do not support "truecolor" and will be more limited, but this
            // may be ok since we are not using rgb format. Only 16 different
            // colors-TJE
            ...
            {
                // assuming this as it seems to still support 16 colors, enough for us
                // today
                terminalType = TERM_LINUX_COLOR;
            }
        }
    ...
    if (!haveCompleteMatch)
    {
        if (get_Environment_Variable("COLORTERM", &termEnv) == ENV_VAR_SUCCESS && termEnv != M_NULLPTR)
        {
            ...
            {
                // centos6 reports "gnome-terminal" which seems to support 256 colors
                // output mode
            ...
            else if (strncmp(termEnv, "vte", 3) == 0)
            {
                ...
                haveCompleteMatch = true;
                 ...
                 // "COLORTERM" is set, assume it has some color output support
            {
                terminalType = TERM_GENERIC_COLOR;
            }
        }
    ...
    if (!haveCompleteMatch)
    {
        ...
        {
            // this environment variable is found in kde for forground and background
            // terminal colors This is probably good enough to say "it has some color"
            // support at this point, but no further indication of what color support
            // is available
            terminalType = TERM_GENERIC_COLOR;
        }
    }
...
typedef struct _consoleColorCap
{
    ...
    // bool useColonInsteadOfSemiColon; //TODO: Implement support for the very few
    // consoles that need a colon instead of semicolon
...
static void get_Console_Color_Capabilities(ptrConsoleColorCap colorCapabilities)
{
    if (colorCapabilities)
    {
        ...
        case TERM_GENERIC_COLOR:
            ...
            break;
        case TERM_UNKNOWN:
            // Assuming color is supported for now until we run into a terminal that
            // does not support any color code changes When this is found we need it
            // in a different case to return this variable
            colorCapabilities->colorSupport = true;
            // if a terminal does not have any way of doing bright color output, set
            // the 8colorsOnly to true. Ex: red/bright red will be the same in these
            // cases
            colorCapabilities->useIntensityBitFormat = true;
            break;
        }
...
void set_Console_Foreground_Background_Colors(eConsoleColors foregroundColor, eConsoleColors backgroundColor)
{
    ...
    if (consoleCap.colorSupport) // if color is not supported, do not do anything
                                 // as the escape sequences will just print to the
                                 // output and make it a mess
    {
        ...
        else
        {
            ...
            //   There are possible needs to adjust color output if this does not work
            //   for an OS we are supporting. Can ifdef or check for other things
            ...
            // Checking for env variable COLORTERM is one method, or COLORFGBG, or if
            // TERM is set to sun-color, xterm-256color, true-color, or gnome-terminal
            // will work for 256color when debugging a unix-like system, try printenv
            // | grep "color" and printenv | grep "COLOR" to see what you can find out
            // about the terminal testing on CentOS 6 through latest Ubuntu the bright
            // colors seem to be supported (codes 90-97 and 100-107) as well as
            // 256color codes In CentOS 6 & 7 the "intensity" field does change to
            // bright colors, but in Ubuntu 22.04 this only changes to a bold font If
            // the extended colors are not available, will need to use the "intensity"
            // method of \033[1:##m to set it instead Sometimes the "intensity" just
            // makes things bold though FreeBSD 11 through 13 claim xterm-256color so
            // they are also likely supported by 256color and extra bright color
            // codes. omniOS r151038 seems to support 256 color codes properly too
            // (sun-color terminal) aixterm also appears to support some color:
            // https://www.ibm.com/docs/en/aix/7.2?topic=aixterm-command, but 256
            // color is not listed reading terminfo or termcap may be the best way to
            // change formats or ignore color changes if not capable at all, but that
            ...
            case CONSOLE_COLOR_DEFAULT:
            default:
                // aixterm does not list this, so will need to test it! otherwise reset
                // with 0m will be as close as we get
                backgroundColorInt = 49;
                break;
            }
            // if background colors do not work, may need to try the "invert" trick to
            // make it happen using a format like \033[7;nm or \033[7;1;nm
            if (backgroundColorInt != UINT8_MAX)
            {
                    ...
                    (consoleCap.use256ColorFormat && consoleCap.eightBackgroundColorsOnly))
                {
                    ...
                    {
                        // more background colors are available using the inversion method
                        // (maybe) convert the color to a foreground color first
...
#    if defined(POSIX_2001)
    ...
    FILE*          term = fopen("/dev/tty", "r"); // use /dev/tty instead of stdin to get
                                                  // the terminal controlling the process.
    bool devtty = true;
...
#    elif (defined(__FreeBSD__) && __FreeBSD__ >= 4 /*4.6 technically*/) || (defined(__OpenBSD__) && defined OpenBSD2_9)
    ...
    // NOTE: Linux's libbsd also provides this, but termios method above is still
    // preferred-TJE
    *inputDataLen = BUFSIZ;
...
#    elif defined(__sun) && defined(__SunOS_5_6)
    ...
    if (eraseme)
    {
        *userInput = strdup(eraseme);
        // immediately clear the original buffer to make sure it cannot be accessed
        // again
        ...
        {
            *inputDataLen = safe_strlen(*userInput) + 1; // add one since this adds to the buffer size and that
                                                         // is what we are returning in all other cases-TJE
        }
    }
    else
    {
...
#    else  // POSIX & OS MACRO CHECKS
    ...
    // which is why it only gets used in this else case as a backup in case the OS
    // we are supporting doesn't support any other available method.-TJE
...
static bool is_Allowed_Unit_For_Get_And_Validate_Input(const char* unit, eAllowedUnitInput unittype)
{
    ...
    if (unit)
    {
        ...
        case ALLOW_UNIT_SECTOR_TYPE:
            if (strcmp(unit, "l") == 0 // used by some utilities to indicate a count is in logical
                                       // sectors instead of physical sectors
                                                    ...
                                                    unsigned long long* outputInteger)
{
    if (strToConvert && outputInteger)
    {
        ...
        if (!ret && unit) // only check for a valid unit if the caller provided an
                          // endptr to get the unit out for their use
        {
            ...
            if (is_Allowed_Unit_For_Get_And_Validate_Input(tmp, unittype))
            {
                ...
                // meaning the user provided 64KB or something like that, so this
                // matched to KB This allows for the utilities calling this to multiply
                // the output integer into a value that makes sense
                ret = true;
            }
        }
                                              ...
                                              float*            outputFloat)
{
    if (strToConvert && outputFloat)
    {
        ...
        errno = 0; // ISO secure coding standard recommends this to ensure errno is
                   // interpretted correctly after this call
                                                       ...
                                                       uint16_t*         outputInteger)
{
    if (strToConvert && outputInteger)
    {
        ...
        {
            ret   = false;
            errno = ERANGE; // manually set a range error since this is outside of
                            // what is expected for this function
...
ssize_t getdelim(char** M_RESTRICT lineptr, size_t* M_RESTRICT n, int delimiter, FILE* stream)
{
    ...
    currentptr = *lineptr;
    endptr     = *lineptr + *n;
    // read using fgetc until delimiter is encountered in the stream or end of the
    // stream is reached
    do
    {
        int c = fgetc(stream);
        ...
        // add to the stream
        *currentptr = C_CAST(char, c); // This cast is necessary to stop a warning.
                                       // C is an int so that EOF can be defined.
        currentptr += 1;
        ...
        // check if writing next two characters will overflow the buffer (next char
...
#    else /*don't have vsnprintf, but we allocated a buffer big enough for this                                        \
             and a NULL terminator, so vsprintf will be safe*/
...
void print_Return_Enum(const char* funcName, eReturnValues ret)
{
    ...
    case UNKNOWN:
        printf("UNKNOWN\n");
        break;
        // NO DEFAULT CASE! This will cause warnings when an enum value is not in
        // this switch-case so that it is never out of date!
    }
    printf("\n");
}
...
static void internal_Print_Data_Buffer(uint8_t* dataBuffer, uint32_t bufferLen, bool showPrint, bool showOffset)
{
    ...
    uint32_t offset      = 0;
    uint32_t offsetWidth = 2; // used to figure out how wide we need to pad with
                              // 0's for consistent output, 2 is the minimum width
    ...
    for (printIter = 0, offset = 0; printIter < bufferLen; ++printIter, ++lineBuffIter)
    {
        ...
        // for every 16 bytes we print, we need to make a newline, then print the
        // offset (hex) before we print the data again
        ...
        if (showPrint && ((printIter + 1) % LINE_WIDTH == 0 || printIter + 1 == bufferLen))
        {
            ...
            lineBuffIter                   = UINT8_MAX; // this is done to cause an overflow when the
                                                        // ++happens during the loop
            if (spacePadding)
            {
                uint32_t counter = 0;
