        ...
        for (LPTSTR envVar_i = envBlock; *envVar_i; envVar_i += _tcslen(envVar_i) + 1)
        {
            for (LPTSTR envVar_j = envVar_i; *envVar_j; envVar_j += _tcslen(envVar_j) + 1)
            {
                if (envVar_i != envVar_j)
                {
                    k = 0;
                    l = 0;
                    // originally used lstrlen, but switched to _tcslen to fix int to
                    // size_t warning
...
M_NODISCARD eEnvVarResult get_Environment_Variable(const char* environmentVariableName, char** envVar)
{
    ...
    *envVar = M_NULLPTR;
    ...
    (defined(__STDC_LIB_EXT1__) && defined(__STDC_WANT_LIB_EXT1__))
        /* MSFT/C11 annex K adds getenv_s, so use it when available to check if this
         * exists */
        size_t size = 0;
         ...
         * DISABLE_SECURE_GETENV is defined by the person building this library. See
         * https://linux.die.net/man/3/secure_getenv for reasons to disable it.
         */
