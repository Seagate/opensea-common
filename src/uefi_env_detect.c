...
eReturnValues get_Operating_System_Version_And_Name(ptrOSVersionNumber versionNumber, char* operatingSystemName)
{
    ...
    {
        snprintf(&operatingSystemName[0], OS_NAME_SIZE, "UEFI - %s - 0x%08" PRIX32, firmwareVendorASCII,
                 gST->FirmwareRevision);
    }
    else
    {
        snprintf(&operatingSystemName[0], OS_NAME_SIZE, "EFI - %s - 0x%08" PRIX32, firmwareVendorASCII,
                 gST->FirmwareRevision);
    }
    return SUCCESS;
}
...
eReturnValues get_Current_User_Name(char** userName)
{
    ...
    if (userName)
    {
        // while unix functions are there, they are all stubs, so we're just going
        // to return "efi" as the username.
