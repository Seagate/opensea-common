...
static eValidateFormatResult update_Format_Offset(const char* format,
...
static eValidateFormatResult check_For_Litteral_Precent(const char* format,
                                                        char**      offsetToSpecifier,
                                                        size_t*     formatoffset,
                                                        size_t      formatLength)
{
...
static eValidateFormatResult validate_Format_Flags(const char* format,
                                                   char**      offsetToSpecifier,
                                                   size_t*     formatoffset,
                                                   size_t      formatLength)
{
...
static eValidateFormatResult validate_Format_Width(const char* format,
                                                   char**      offsetToSpecifier,
                                                   size_t*     formatoffset,
                                                   int*        width,
                                                   va_list*    args,
                                                   size_t      formatLength)
{
    ...
    if (format && offsetToSpecifier && formatoffset && width && args)
    {
        ...
        {
            *width = va_arg(*args, int); // read this to move to next thing in arg
                                         // list
            *offsetToSpecifier += 1;
            result = update_Format_Offset(format, *offsetToSpecifier, formatoffset, formatLength);
        }
        else
        {
            ...
            {
                // something went wrong reading the width specifier, so treat this as an
                // error
                result = VALIDATE_FORMAT_INVALID_FORMAT;
            }
            else
            {
                *width = C_CAST(int, lwidth);
                // read the width, endptr should be pointing to the next part we need to
                // look at
...
static eValidateFormatResult validate_Format_Precision(const char* format,
                                                       char**      offsetToSpecifier,
                                                       size_t*     formatoffset,
                                                       int*        precision,
                                                       va_list*    args,
        ...
        if ((*offsetToSpecifier)[0] == '.')
        {
            *offsetToSpecifier += 1;
            ...
            else
            {
                ...
                {
                    // something went wrong reading the width specifier, so treat this as
                    // an error
                    return VALIDATE_FORMAT_INVALID_FORMAT;
                }
                else
                {
                    *precision = C_CAST(int, lprecision);
                    // read the width, endptr should be pointing to the next part we need
                    // to look at
                    *offsetToSpecifier = endptr;
                    result             = update_Format_Offset(format, *offsetToSpecifier, formatoffset, formatLength);
                    if (result == VALIDATE_FORMAT_COMPLETE)
...
static eValidateFormatResult validate_Format_Length_Modifier(const char*                    format,
                                                             char**                         offsetToSpecifier,
                                                             size_t*                        formatoffset,
...
static eValidateFormatResult validate_Format_Integer(const char*                    format,
                                                     char**                         offsetToSpecifier,
                                                     size_t*                        formatoffset,
                                                     ...
                                                     va_list*                       args)
{
    ...
    if (format && offsetToSpecifier && formatoffset && lenmods && args)
    {
        ...
        if ((*offsetToSpecifier)[0] == 'd' || (*offsetToSpecifier)[0] == 'i')
        {
            ...
            {
                signed char value =
                    C_CAST(signed char, va_arg(*args,
                                               int)); // type promoted to int in va_args, so access as int
                M_USE_UNUSED(value);
            }
            else if (lenmods->h == true)
            {
                signed short value =
                    C_CAST(signed short, va_arg(*args,
                                                int)); // type promoted to int in va_args, so access as int
                M_USE_UNUSED(value);
            }
        ...
        else // unsigned
        {
            ...
            {
                unsigned char value =
                    C_CAST(unsigned char, va_arg(*args, unsigned int)); // type promoted to unsigned int in
            ...
            else if (lenmods->h == true)
            {
                unsigned short value =
                    C_CAST(unsigned short, va_arg(*args, unsigned int)); // type promoted to unsigned int in
                                                                         // va_args, so access as unsigned int
                M_USE_UNUSED(value);
            }
            else if (lenmods->ll == true)
            ...
            {
                // unsigned version of this....but this should be ok since we aren't
                // doing anything else with this right now
                ptrdiff_t value = va_arg(*args, ptrdiff_t);
                M_USE_UNUSED(value);
            }
            else
...
static eValidateFormatResult validate_Format_Float(const char*                    format,
                                                   char**                         offsetToSpecifier,
                                                   size_t*                        formatoffset,
                                                   ptrVerifyFormatLengthModifiers lenmods,
                                                   va_list*                       args)
{
    ...
    if (format && offsetToSpecifier && formatoffset && lenmods && args)
    {
        // NOTE: lowercase l is also allowed but only means double which is why it
...
static eValidateFormatResult validate_Wchar_Conversion(wint_t widechar)
{
    ...
    {
        return VALIDATE_FORMAT_INVALID_FORMAT; // Should this be a different error
    ...
    errno = 0; // ISO C does not show this as necessary, but doing to set this to
               // zero anyways
#if defined(__STDC_SECURE_LIB__) || defined(HAVE_C11_ANNEX_K)
    // wcrtombs_s
...
static eValidateFormatResult validate_Format_Char(const char*                    format,
                                                  char**                         offsetToSpecifier,
                                                  size_t*                        formatoffset,
                                                  ptrVerifyFormatLengthModifiers lenmods,
                                                  va_list*                       args)
{
    ...
    if (format && offsetToSpecifier && formatoffset && lenmods && args)
    {
        if (lenmods->l == true)
        {
            ...
            // va_args based on what size it is. va_args promote smaller types to int,
            // but wint_t may be smaller or the same size which is why this check is
            // here.
...
static eValidateFormatResult validate_Format_Pointer(const char*                    format,
                                                     char**                         offsetToSpecifier,
                                                     size_t*                        formatoffset,
                                                     ptrVerifyFormatLengthModifiers lenmods,
                                                     va_list*                       args)
...
static eValidateFormatResult validate_Format_String(const char*                    format,
                                                    char**                         offsetToSpecifier,
                                                    size_t*                        formatoffset,
                                                    ptrVerifyFormatLengthModifiers lenmods,
                                                    va_list*                       args)
...
static eValidateFormatResult validate_Format_Specifier(const char*                    format,
                                                       char**                         offsetToSpecifier,
                                                       size_t*                        formatoffset,
                                                       size_t                         formatlength,
                                                       ptrVerifyFormatLengthModifiers lenmods,
                                                       va_list*                       args)
{
    ...
    if (format && offsetToSpecifier && formatoffset && lenmods && args)
    {
        ...
        default:
            ...
            // We can add handling for platform unique formats, but they should only
            // be allowed on that platform. For example, if it is Windows unique, it
            // should only be allowed in Windows, POSIX unique = only POSIX, etc
            result = VALIDATE_FORMAT_INVALID_FORMAT;
            break;
        }
...
int verify_Format_String_And_Args(const char* M_RESTRICT format, va_list formatargs)
{
    if (format)
    {
        char* offsetToSpecifier = strstr(format, "%"); // if there are no formatting specifiers, just skip
                                                       // all the checks to return zero
        size_t formatoffset = 0;
        if (offsetToSpecifier != M_NULLPTR)
        {
            va_list args;
            size_t  formatLength = 0;
            // removed early %n detection since it would incorrectly catch %%n which
            // is a valid sequence. The rest of the code does catch %n and modifiers
            // to it, so it is safe to let it proceed. see answer from S.S. Anne here
            // for why it is necessary to make a copy of the args:
            ...
            // The only other option we have is to try a simple assignment which may
            // or may not work as expected This is what MSVC defines the va_copy macro
            // as, but other compilers may handle this some other way. If we need to
            // define other compiler specific versions to make this work, we can! -TJE
            args = formatargs;
#endif
            formatoffset = C_CAST(uintptr_t, offsetToSpecifier) - C_CAST(uintptr_t, format);
            formatLength = safe_strnlen(format, C_STR_LITERAL_LIMIT);
            // check at least the strings to make sure they are non-M_NULLPTR.
            // checking for other encoding errors may be more difficult to detect in
            // here -TJE
            bool exitloop = false;
            while (offsetToSpecifier != M_NULLPTR && formatoffset < formatLength && exitloop == false)
            {
                ...
                // rather than go through parsing all of this, update the offset and
                // continue the loop
                ...
                // M_VALIDATE_FORMAT_RETURN_VAL works it may not exit a switch-case
                //       the way we want. It will process as expected for if-else though
                //       which is why we are doing this.-TJE
                M_VALIDATE_FORMAT_RETURN_VAL(
        ...
        return C_CAST(int,
                      formatoffset); // This should be safe as the format string
