...
// same CPU type in order to simplify this code more - TJE
    ...
    //       not needed. We already natively support ARM and AARCH64, so there is
    //       no need to compile this code in this compatibility mode.-TJE
...
{
    static eEndianness endian = OPENSEA_UNKNOWN_ENDIAN; // using static so that it should only need to run
                                                        // this code once...not that it takes a long time,
                                                        // but this may help optimise this.
...
#        elif defined(__ARMEL__) || defined(__THUMBEL__) || defined(__AARCH64EL__) || defined(_MIPSEL) ||              \
            ...
            defined _M_PPC /* This is a special Windows case for PPC as NT ran it in                                   \
                              little endian mode */                                                                    \
...
{
    eReturnValues ret = SUCCESS; // should always return this unless we don't find
                                 // a compiler we support.
...
#    define INTEL_VERSION_STRING_MAX_LENGTH 4;
    ...
    *compilerUsed              = OPENSEA_COMPILER_SUNPRO_C_CPP;
    ...
    compilerVersionInfo->minor =
        (M_Nibble2(__SUNPRO_C) << 4) | M_Nibble1(__SUNPRO_C); // I think this is wrong. Version 5.10 is represented as
                                                              // 0x5100. Need to put extra code in here to fix this
                                                              // when we actually support this compiler.
    ...
    *compilerUsed              = OPENSEA_COMPILER_GCC;
    ...
    compilerVersionInfo->patch = __GNUC_PATCHLEVEL__; // introduced in GCC 3.0, but even that is super old
                                                      // so this shouldn't cause any problems.-TJE
#else
    ret = NOT_SUPPORTED;
#endif
    return ret;
