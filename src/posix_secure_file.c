...
static uid_t get_sudo_uid(void)
{
    ...
    if (!gotsudouid)
    {
        ...
        if (get_Environment_Variable("SUDO_UID", &uidstr) == ENV_VAR_SUCCESS && uidstr != M_NULLPTR)
        {
            ...
            errno = 0; // clear before calling this function as recommended by ISO C
                       // secure coding
...
#    endif
        ...
        {
            /* Before we assume insecure, check if this was executed as sudo by the
             * user as this directory may be the user's directory and ok to access */
...
#    endif
            ...
            else
            {
                ...
                secure = false;
#    if defined(_DEBUG)
                printf("Directory owned by someone other than user or root: %u my_uid: "
                       "%u\n",
                       buf.st_uid, my_uid);
#    endif
                break;
