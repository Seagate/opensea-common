...
// obtain the \\StringFileInfo\\<lang><codepage>\\ProductVersion subblock of the
// file version information. NOTE: Changed from GetFileVersionInfo as it did not
...
eReturnValues read_Win_Version(ptrOSVersionNumber versionNumber)
{
    ...
    if (!readVersionFromNTDLL)
    {
        ...
        // I'm using this Microsoft provided call to concatenate strings since it
        // will concatenate properly for ansi or wide strings depending on whether
        // UNICODE is set or not - TJE
        ...
        if (TRUE == GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, systemPathBuf, &hMod))
        {
            if (hMod != INVALID_HANDLE_VALUE)
            {
                ...
                if (rtlgetverptr != M_NULLPTR)
                {
                    ...
                    if (rtlgetverptr(&osInfo) >= 0) // not using NT_SUCCESS from ntdef.h because it causes lots or
                                                    // redefinition errors, but this is all that macro does
                    {
...
bool is_Windows_8_One_Or_Higher(void)
{
    ...
    conditionMask = 0;
    //Now get the actual version of the OS...start at windows vista and work
    forward from there. windowsVersionInfo.dwMajorVersion = 6;
    ...
    VER_MINORVERSION, VER_GREATER_EQUAL); if
    (VerifyVersionInfo(&windowsVersionInfo, VER_MAJORVERSION | VER_MINORVERSION,
    conditionMask))
    {
    ...
    else if (windowsVersion.versionType.windowsVersion.majorVersion == 6 &&
...
bool is_Windows_10_Or_Higher(void)
{
    ...
    conditionMask = 0;
    //Now get the actual version of the OS...start at windows vista and work
    forward from there. windowsVersionInfo.dwMajorVersion = 10;
    windowsVersionInfo.dwMinorVersion = 0;
    conditionMask = VerSetConditionMask(conditionMask, VER_MAJORVERSION,
    VER_GREATER_EQUAL); conditionMask = VerSetConditionMask(conditionMask,
    VER_MINORVERSION, VER_GREATER_EQUAL); if
...
bool is_Windows_11_Version_24H2_Or_Higher(void)
{
    ...
    read_Win_Version(&windowsVersion);
    if (windowsVersion.versionType.windowsVersion.majorVersion >= 10 &&
        windowsVersion.versionType.windowsVersion.buildNumber >= 26100)
    {
...
bool is_Windows_PE(void)
{
    ...
    // To figure out if running in PE requires checking the registry. There is not
    // another known way to look for this. All other functions to read or check
    // versions will return build numbers that match the equivalent non-PE version
    // of windows, which can still be useful for determining supported APIs We
    // will be looking for one of these keys:
    /*
    ...
    if (!regread)
    {
        ...
        if (!isWindowsPE &&
            ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                                          TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\WinPE"), 0, KEY_READ,
                                          &keyHandle))
        {
#if defined(_DEBUG)
