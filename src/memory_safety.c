...
size_t get_System_Pagesize(void)
{
    ...
    // note marked legacy in SUSv2, but checking it if posix is missing for some
    // reason
    ...
    return 4096; // most CPUs use a 4KiB page size. We can detect specific
                 // architectures using something different if we need to here-TJE
#endif
}
...
M_FUNC_ATTR_MALLOC void* malloc_aligned(size_t size, size_t alignment)
{
    ...
    // multiple of alignment, so we will check and round up the size to make this
    // easy.
    if (size % alignment)
    {
    ...
    // use the MS _aligned_malloc function. Mingw64 will support this as well from
    // what I can find - TJE
    ...
    // necessary NOTE: May need to include malloc.h on some systems if this is not
    // a known function from stdlib.h where it is sometimes included from too-TJE
    ...
    // and adding to get to the required alignment...then knowing how much we over
    // aligned by. Will store the original starting pointer right before the
    // aligned pointer we return to the caller.
    void* temp = M_NULLPTR;
    ...
    {
        size_t requiredExtraBytes = sizeof(size_t); // We will store the original beginning address in front
                                                    // of the return data pointer
        ...
        if (temp)
        {
            ...
            temp += requiredExtraBytes; // allow enough room for storing the original
                                        // pointer location
...
bool is_Empty(const void* ptrData, size_t lengthBytes)
{
    if (ptrData || lengthBytes == 0)
    {
        ...
        // Allocate single memory page as zeroes to compare for the range instead of
        // byte by byte for better performance.-TJE
        ...
        if (sysPageSize != 0)
        {
            ...
            if (zeroes)
            {
                ...
                for (size_t iter = 0; iter < lengthBytes; iter += increment)
                {
                    ...
                    // update incement amount to ensure this does not access invalid
                    // memory
                }
                safe_Free_page_aligned(C_CAST(void**, &zeroes));
            }
...
#    endif
    ...
    defined(HAVE_EXPLICIT_MEMSET)
        ...
        // NOTE: Solaris 11.4.12 added this, but I cannot find it in illumos based
        // distributions
        //       Illumos does not list this, but lists explicit_bzero in their
        //       manual. Not sure what version to use, so letting meson detect and
        //       set the HAVE_...macros
...
#            if !defined(NO_HAVE_MSFT_SECURE_ZERO_MEMORY2) &&                                                          \
                (defined(HAVE_MSFT_SECURE_ZERO_MEMORY2) ||                                                             \
                 (defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN11_26100))
        // SecureZeroMemory2 calls FillVolatileMemory which we can use here to do
        // the same thing
...
#                endif
#            else
        /*if you hit this case for some reason, you will need to add an include for
         * <intrin.h>. Not currently done as SecureZeroMemory is used instead*/
        /* https://learn.microsoft.com/en-us/cpp/intrinsics/readwritebarrier?view=msvc-170
         */
        memset(dest, ch, count);
        _ReadWriteBarrier();
#            endif
#        else /* compiler does not support a method above as a barrier, so use this as                                 \
                 a final way to try and prevent optimization */
        // one idea on the web is this ugly volatile function pointer to memset to
        // stop the compiler optimization
...
M_FUNC_ATTR_MALLOC void* safe_realloc(void* block, size_t size)
{
    ...
    else
    {
        ...
        // a simple return realloc, the purpose of this is to help reduce false
        // positives with SAST tools.
...
static size_t alignment_Round_Up(size_t alignment)
{
    ...
    // than this doesn't make sense to align memory for as anything less will also
    // be compatible with void*
...
M_FUNC_ATTR_MALLOC void* safe_calloc_aligned(size_t count, size_t size, size_t alignment)
{
    ...
    {
        // instead of calling calloc_aligned, call safe_Malloc_aligned since it will
        // round alignment and size for us.
        void*  zeroedMem = M_NULLPTR;
...
errno_t safe_memmove(void* dest, rsize_t destsz, const void* src, rsize_t count)
{
    ...
    {
        // https://en.cppreference.com/w/c/string/byte/memmove for what to do on
        // error
...
#    if defined(__STDC_SECURE_LIB__)
    // add detection for overlapped range and destsz & count <= RSIZE_MAX since
    // MSFT's implementation does not do this
    if (destsz <= RSIZE_MAX && count <= RSIZE_MAX &&
        !(src <= dest && C_CAST(void*, C_CAST(uintptr_t, src) + count) > dest))
    {
...
#    endif
    ...
    // make sure all parameters match the spec requirements, including detection
    // of an overlapping range. if the range overlaps, then memmove should be used
    // instead.
    if (dest && src && destsz <= RSIZE_MAX && count <= RSIZE_MAX && count <= destsz &&
        !(src <= dest && C_CAST(void*, C_CAST(uintptr_t, src) + count) > dest))
    {
    ...
    {
        // https://en.cppreference.com/w/c/string/byte/memcpy for what to do on
        // error
        if (dest && destsz > 0 && destsz <= RSIZE_MAX)
        {
            memset(dest, 0, destsz);
