...
static void test_ROUNDF(void) {
    TEST_ASSERT_EQ(ROUNDF( FLOAT32_MIN_VAL, 100), -340282346638528859811704183484516925440.00f, "ROUNDF of FLOAT32_MIN_VAL to 2 decimal places");
    TEST_ASSERT_EQ(ROUNDF( FLOAT32_MAX_VAL, 1000), 340282346638528859811704183484516925440.000f, "ROUNDF of FLOAT32_MAX_VAL to 0 decimal places");
    TEST_ASSERT_EQ(ROUNDF(FLOAT32_MIN_PLUS1, 10), -340282346638528859811704183484516925439.0000000000f, "ROUNDF of FLOAT32_MIN_PLUS1 to 1 decimal places");
    TEST_ASSERT_EQ(ROUNDF(FLOAT64_MIN_VAL, 100), -.00, "ROUNDF of FLOAT64_MIN_VAL to 2 decimal places");
    TEST_ASSERT_EQ(ROUNDF(FLOAT64_MAX_VAL, 1000), FLOAT64_MAX_VAL * 1.000f, "ROUNDF of FLOAT64_MAX_VAL to 0 decimal places");
}
...
struct sgenericint_t INVALID_GENERIC_INT_WITH_FLAG_MISMATCH = {
    .issigned = false,
    .sizeoftype = sizeof(int32_t),
    .i32 = -100
};

struct sgenericint_t INVALID_GENERIC_INT_WITH_OVERFLOW = {
    .issigned   = false,
    .sizeoftype = sizeof(uint8_t),
    .u8         = 300
};
