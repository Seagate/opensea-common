...
extern "C"
{
    ...
    //
    //! \brief   Description:  Fills a buffer with random data. Data length must be
    //! at least sizeof(uint32_t) in size (4 bytes)
    //
    //  Entry:
    //!   \param[out] ptrData = pointer to the data buffer to fill
    //!   \param[in] dataLength = size of the data buffer in bytes (must be at least
    //!   4 bytes in size)
    //!
    //  Exit:
    ...
    //-----------------------------------------------------------------------------
    //
    //  fill_Hex_Pattern_In_Buffer(uint32_t hexPattern, uint8_t *ptrData, uint32_t
    //  dataLength)
    //
    //! \brief   Description:  Fills a buffer with a hexadecimal pattern. The
    //! pattern is 32bits in length.
    //
    //  Entry:
    ...
    //-----------------------------------------------------------------------------
    //
    //  fill_Incrementing_Pattern_In_Buffer(uint8_t incrementStartValue, uint8_t
    //  *ptrData, uint32_t dataLength)
    //
    //! \brief   Description:  Fills a buffer with a incrementing pattern from a
    //! starting point
    //
    //  Entry:
    ...
    //-----------------------------------------------------------------------------
    //
    //  fill_ASCII_Pattern_In_Buffer(const char *asciiPattern, uint32_t
    //  patternLength, uint8_t *ptrData, uint32_t dataLength)
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  fill_Pattern_Buffer_Into_Another_Buffer(uint8_t *inPattern, uint32_t
    //  inpatternLength, uint8_t *ptrData, uint32_t dataLength)
    //
    //! \brief   Description:  Fills a buffer from a pattern memory buffer and will
    //! repeat the pattern to fill the out buffer. (i.e., 1 sector in gets copied
    //! and put into all 4 sectors out)
    //
    //  Entry:
