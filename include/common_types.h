...
#    elif defined(NULL)
                        // use NULL since this is commonly available and likely to be safe
...
#    else
                        // for unknown reasons M_NULLPTR was not defined so define it the
                        // most common way we can to be safe
...
#define ROOT_UID_VAL (0)
    ...
    M_DECLARE_ENUM(eReturnValues,
                   ...
                   FILE_OPEN_ERROR       = 12,
                   WARN_INCOMPLETE_RFTRS = 13, /* command was issued, and some RTFRs were received, but we were
                                                  unable to get a complete RTFR result. This is most likely due to
                                                  a SATL limitation. */
...
#            pragma clang diagnostic ignored "-Wc++0x-compat" // treated as synonymn for Wc++11-compat, so use this for
