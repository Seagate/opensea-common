...
#define FILE_UNIQUE_ID_ARR_MAX (16)
    ...
    M_PARAM_RO(1) fileUniqueIDInfo* M_NULLABLE os_Get_File_Unique_Identifying_Information(FILE* M_NONNULL file);
    ...
    //! within it.
    M_NODISCARD M_PARAM_RO(1) M_PARAM_WO(2) bool os_Is_Directory_Secure(const char* M_NONNULL        fullpath,
                                                                        char* M_NULLABLE* M_NULLABLE outputError);
    ...
    typedef struct sfileAttributes
    {
        ...
        char* M_NULLABLE winSecurityDescriptor;
#endif // _WIN32
    } fileAttributes;
    ...
    M_NODISCARD_REASON("The returned pointer must be freed by the caller using free_File_Attributes()")
    M_PARAM_RO(1) fileAttributes* M_NULLABLE os_Get_File_Attributes_By_Name(const char* M_NONNULL filetoCheck);
    ...
    //! \note The returned pointer must be freed by the caller using free_File_Attributes().
    M_NODISCARD_REASON("The returned pointer must be freed by the caller using free_File_Attributes()")
    M_PARAM_RO(1) fileAttributes* M_NULLABLE os_Get_File_Attributes_By_File(FILE* M_NONNULL file);
    ...
    M_PARAM_RW(1) eSecureFileError secure_Flush_File(secureFileInfo* M_NONNULL fileInfo);
    ...
    M_PARAM_RO(1) bool os_Directory_Exists(const char* M_NONNULL pathToCheck);
    ...
    //! \note WARNING: May not work with UNICODE path.
    M_PARAM_RO(1) bool os_File_Exists(const char* M_NONNULL filetoCheck);
    ...
    M_PARAM_RO(1) int64_t os_Get_File_Size(FILE* M_NONNULL filePtr);
    ...
    M_PARAM_RO(1)
    M_PARAM_RO(2)
    bool exact_Compare_SIDS_And_DACL_Strings(const char* M_NONNULL sidsAndDACLstr1,
                                             const char* M_NONNULL sidsAndDACLstr2);
#endif
