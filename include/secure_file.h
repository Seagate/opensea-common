...
#define FILE_UNIQUE_ID_ARR_MAX (16)
    typedef struct _fileUniqueIDInfo
    {
        ...
        // Windows uses 2 unsigned longs for index high/low. NOTE: REFS needs a 128bit
        // identifier, need to use ex version to read full identifier Linux has ino_t
        // and dev_t to check for Idea: Both linux and windows recommend combing
        // things to create a unique ID (Windows: Vol SN + file index, Linux: ino_t +
        // dev_t)
        //       use a very generic thing to compare that can hold this many bytes of
        //       data...an array. low-level OS function to do the comparison???
        union
        {
            uint64_t inode;
            uint64_t volsn;
    ...
    typedef struct _secureFileInfo
    {
        ...
        bool             isValid;
        FILE*            file;
        const char       fullpath[OPENSEA_PATH_MAX]; // contains the full canonicalized path
                                                     // and filename.
        const char* filename;                        // pointer to just the name section in the fullpath
                                                     // above
