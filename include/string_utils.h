...
// it has the built - in function with this macro
// instead. This was added to GCC 10, but other GCC compatible compilers may use
// this if not defining a compatible GCC versio
...
#    endif
    ...
    // heapapi with the HeapSize function, however I cannot get it to work. It is
    // always crashing internally, so we are not going to use it to try and make
    // sure the maximum is restricted at this time -TJE
    ...
    //  const char* source);
    //
    //! \brief   Description:  To be used in place of strcat. This will work more
    //! like strlcat in the BSDs and will always null terminate.
    //
    //  Entry:
    ...
    //
    //! \brief   Description:  To be used in place of strncat. Will always null
    ...
    //
    //-----------------------------------------------------------------------------
    char* common_String_Concat_Len(char*       destination,
