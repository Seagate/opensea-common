...
extern "C"
{
    ...
    //
    // \brief   Description: returns an enum value specifying which processor
    // architecture opensea-common (and likely the rest of opensea-* libs) was
    // compiled for.
    //
    // Entry:
    //
    ...
    //
    // \brief   Description: Prints out the architecture in human readable form
    // (most likely short hand, ie: powerpc = PPC)
    //
    // Entry:
    ...
    M_DECLARE_ENUM(eEndianness,
                   ...
                   OPENSEA_UNKNOWN_ENDIAN      /*if this is returned, then __LITTLE_ENDIAN__ or
                                                  __BIG_ENDIAN__ needs to be defined for      the
                                                  compilation. Or you can ignore this and assume
                                                  little endian - TJE*/
    );
    ...
    //
    // \brief   Description: returns an enum value specifying which endianness was
    // detected when it was compiled.
    ...
    //
    // \brief   Description: Prints out the endianness in human readable form. I.E.
    // LSB or MSB for short and Little Endian and Big Endian for long
    //
    // Entry:
    //      \param[in] endian - eEndianness type
    //      \param[in] shortPrint - print out 3 digit short hand endianness or long
    ...
    M_DECLARE_ENUM(eOSType,
                   ...
                   OS_HPUX,
                   OS_ESX,
                   /*Add more operating systems here as we add support for them in
                      our libraries*/
    );
...
// very large due to utsname potentially having different sizes for some fields:
// https://man7.org/linux/man-pages/man2/uname.2.html
#define OS_NAME_SIZE (512)

    //-----------------------------------------------------------------------------
    //
    // get_Operating_System_Version_And_Name(eOSType *osType, ptrOSVersionNumber
    // versionNumber, char *operatingSystemName)
    //
    // \brief   Description: Gets the version number of the OS opensea-* libs are
    // currently running on. Windows will get the OS version number (not Win7 vs
    // Win8, but 6.1 vs 6.2). Nix systems will get whatever is placed into uname -r
    // (solaris will also get uname -v information)
    //
    // Entry:
    //      \param[out] versionNumber - pointer to the OSVersionNumber struct. This
    //      will be filled in with version information upon SUCCESSful completion.
    //      \param[out] operatingSystemName - (optional, set to M_NULLPTR if not
    //      being used) This will be a string with the friendly, human readable name
    //      of the OS. This will likely be the name of the release as it was
    //      announced. IE: Windows 6.2 = Windows 8.
    //
    // Exit:
    //      \return SUCCESS = got version information, !SUCCESS = failure getting
    //      version information.
    //
    //-----------------------------------------------------------------------------
    ...
    //
    // \brief   Description: Prints out the OS Version number. This will attempt to
    // match the was version numbers are represented by each OS (typically #.#.#)
    //
    // Entry:
    //      \param[in] versionNumber - pointer to the OSVersionNumber structure with
    //      version information to print out
    //
    // Exit:
    //      \return void
    //
    ...
    M_DECLARE_ENUM(eCompiler,
                   ...
                   /*Add other compilers here if we ever add more than those above
                      (which not all listed above are supported!)*/
    );
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_Compiler_Info(eCompiler *compilerUsed, ptrCompilerVersion
    //  compilerVersionInfo)
    //
    //! \brief   Description:  This call will return which compiled and version of
    //! that compiler was used when compiling opensea-common (and likely the rest of
    //! opensea-* libs)
    //
    //  Entry:
    //!   \param[out] compilerUsed = pointer to a eCompiler type that will be set
    //!   with an enum value representing the compiler that was used on successful
    //!   completion.. \param[out] compilerVersionInfo = pointer to the
    //!   compilerVersion struct. This will be filled in with version information on
    ...
    //
    //! \brief   Description:  This takes an eCompiler type and prints out the name
    //! of the compiler.
    //
    //  Entry:
    //!   \param[in] compilerUsed = eCompiler type that will be printed to the
    //!   screen in human readable form
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  prints out the compiler version information in the
    //! form major.minor.patch from the compilerVersion struct
    //
    //  Entry:
    //!   \param[in] compilerVersionInfo = pointer to the compilerVersion struct
    //!   that holds the compiler version information.
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //  is_Running_Elevated
    //
    //! \brief   Description:  Function that checks if the process is currently
    //! running with elevated permissions or not. Useful since things like disc
    //! access require this.
    //
    //  Entry:
    //!
    //  Exit:
    //!   \return true = is elevated permissions, false = not running with elevated
    //!   permissions
    //
    //-----------------------------------------------------------------------------
...
#    endif //_WIN32
    ...
    //
    //! \brief   Description:  This function will look up the current user name
    //! using getuid(). This function allocates memory for the user name, so make
    //! sure to free it when it is done.
    //!                        In linux/unix systems this uses an API that opens the
    //!                        /etc/passwd file to do this mapping.
    //
    //  Entry:
    //!
    //  Exit:
    //!   \return SUCCESS = no errors, userName allocated and ready to be user,
    //!   BAD_PARAMTER = bas pointer, FAILURE = could not determine user name.
    //
    //-----------------------------------------------------------------------------
